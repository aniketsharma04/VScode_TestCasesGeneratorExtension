...
const { PythonTestFixer } = require('./python-test-validator-fixer');
const PythonAIPromptGenerator = require('./python-ai-prompt-generator');
// --- PYTHON TEST GENERATION WORKFLOW ---
/**
 * Generate Python tests using AI, fixer, and validation pipeline
 */
export async function generatePythonTests(sourceFilePath: string, config: ExtensionConfig) {
    const fs = require('fs');
    const path = require('path');
    let sourceCode = fs.readFileSync(sourceFilePath, 'utf-8');
    // Ensure sourceCode is a string (in case encoding is omitted elsewhere)
    ...
    const fixer = new PythonTestFixer(sourceFilePath, '');
    fixer.sourceContent = sourceCode;
    fixer.analyzeSourceFile();

    // Step 2: Generate optimized prompt
    const prompt = PythonAIPromptGenerator.generatePrompt(
        sourceCode,
        fileName,
        { testCount: 10 }
    );

    // Step 3: Call AI (Gemini or Claude)
    let aiResponse;
    if (config.apiProvider === 'anthropic') {
        aiResponse = await generateWithClaudeUsingPrompt(prompt, config);
    } else {
        aiResponse = await generateWithGeminiUsingPrompt(prompt, config);
    }

    // Step 4: Quality check
    let truncationCheck = PythonAIPromptGenerator.detectTruncation(aiResponse);
    if (truncationCheck.score < 50) {
        console.log('⚠️  Low quality, retrying...');
        // Retry with stricter prompt
        const strictPrompt = PythonAIPromptGenerator.generatePrompt(
            sourceCode,
            fileName,
            { testCount: 6 }
        );
        if (config.apiProvider === 'anthropic') {
            aiResponse = await generateWithClaudeUsingPrompt(strictPrompt, config);
        } else {
            aiResponse = await generateWithGeminiUsingPrompt(strictPrompt, config);
        }
        truncationCheck = PythonAIPromptGenerator.detectTruncation(aiResponse);
    }

    // Step 5: Fix the generated tests
    const fixerInstance = new PythonTestFixer(sourceFilePath, aiResponse);
    const result = await fixerInstance.fix();

    if (!result.success) {
        throw new Error(`Failed to fix Python tests: ${result.error}`);
    }

    // Step 6: Save test file
    const testFileName = `test_temp_${Date.now()}.py`;
    const testFilePath = path.join(path.dirname(sourceFilePath), testFileName);
    fs.writeFileSync(testFilePath, result.content);

    console.log(`✅ Python tests generated: ${testFileName}`);
    console.log(`   Quality score: ${truncationCheck.score}/100`);

    return {
        success: true,
        testFilePath,
        qualityScore: truncationCheck.score,
...
export async function generateTestsWithOrchestrator(
    ...
    if (language === 'python') {
        // Use the new Python workflow
        const result = await generatePythonTests(sourceFilePath, config);
        return {
            language: 'python',
            framework: 'pytest',
            testCases: [], // Optionally, parse test cases from result.content if needed
            imports: '',
    ...
    // Default: JavaScript/TypeScript/Java orchestrator
    const testFramework = framework || getDefaultFramework(language);
